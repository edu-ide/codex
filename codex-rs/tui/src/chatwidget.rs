...
pub(crate) struct ChatWidget {
    ...
    last_thread_session: Option<ThreadSessionState>,
...
impl ChatWidget {
    ...
    fn on_session_configured_with_display_and_fork_parent_title(
        ...
        self.last_thread_session = Some(session.clone());
    ...
    pub(crate) fn handle_server_notification(
        ...
        match notification {
            ...
            ServerNotification::ModelRerouted(notification) => self.on_model_rerouted(notification),
    ...
    fn on_model_rerouted(
        ...
        self.session_header.set_model(&notification.to_model);
        self.refresh_model_display();
        self.refresh_status_surfaces();

        if let Some(mut session) = self.last_thread_session.clone() {
            session.model = notification.to_model.clone();
            self.last_thread_session = Some(session.clone());
            let show_fast_status =
                self.should_show_fast_status(&notification.to_model, self.effective_service_tier);
            let session_info_cell = history_cell::new_session_info(
                &self.config,
    ...
    fn model_display_name(&self) -> &str {
        if let Some(model) = self.display_model_override.as_deref() {
            return model;
        }
        let model = self.current_model();
        if model.is_empty() {
