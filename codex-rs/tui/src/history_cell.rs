    ...
    if is_first_event {
        ...
        let help_lines: Vec<Line<'static>> = vec![
            ...
            Line::from(vec![
                "  ".into(),
                "/permissions".into(),
                " - choose what Ilhae is allowed to do".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/model".into(),
...
impl HistoryCell for SessionHeaderHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        // Title line rendered inside the box: ">_ Ilhae (vX)"
...
impl HistoryCell for LoopLifecycleCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        let bullet = match self.status {
            LoopLifecycleStatus::InProgress => activity_indicator(
                Some(self.start_time),
                MotionMode::from_animations_enabled(self.animations_enabled),
                ReducedMotionIndicator::StaticBullet,
            )
    ...
    fn raw_lines(&self) -> Vec<Line<'static>> {
        ...
        if self.summary.trim().is_empty() {
            ...
            lines.push(Line::from(format!("{header} {}", self.summary)));
        }
        ...
        if let Some(detail) = self
            ...
            .filter(|value| !value.trim().is_empty())
        {
            lines.push(Line::from(detail.trim().to_string()));
        }
        if let Some(error) = self
            .error
            .as_deref()
            .filter(|value| !value.trim().is_empty())
        {
            lines.push(Line::from(format!("Error: {}", error.trim())));
        }
        lines
    }
...
mod tests {
    ...
    fn web_search_history_cell_wraps_with_indented_continuation() {
        ...
        assert_eq!(
            rendered,
            vec![
                "• Searched the web example search query with several generic".to_string(),
    ...
    fn web_search_history_cell_short_query_does_not_wrap() {
        ...
        assert_eq!(rendered, vec!["• Searched the web short query".to_string()]);
    }
