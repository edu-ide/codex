...
macro_rules! register_artifact_tools {
    ($builder:expr, $brain_service:expr, $bt_settings:expr, $connection_sessions:expr) => {{
        use $crate::{
        ...
        $builder
            .tool_fn(
                 ...
                 The system automatically determines the file path and version.",
                {
                    ...
                    let connection_sessions = $connection_sessions.clone();
                    async move |input: ArtifactSaveInput, cx: sacp::mcp_server::McpConnectionTo<sacp::Conductor>| {
                        $crate::check_tool_enabled!(bts, "artifact_save");

                        let sid = connection_sessions
                            .get(&$crate::shared_state::connection_key(&cx.connection_to()))
                            .unwrap_or_default();
                        if sid.is_empty() {
            ...
            .tool_fn(
                ...
                "List all artifacts in the current session with version info.",
                {
                    ...
                    let bts = $bt_settings.clone();
                    let connection_sessions = $connection_sessions.clone();
                    async move |_input: ArtifactListInput, cx: sacp::mcp_server::McpConnectionTo<sacp::Conductor>| {
            ...
            .tool_fn(
                 ...
                 Returns the latest version by default, or a specific version if specified.",
                {
                    ...
                    let bts = $bt_settings.clone();
                    let connection_sessions = $connection_sessions.clone();
                    async move |input: ArtifactGetInput, cx: sacp::mcp_server::McpConnectionTo<sacp::Conductor>| {
            ...
            .tool_fn(
                 ...
                 refine plans, or add to walkthroughs.",
                {
                    ...
                    let bts = $bt_settings.clone();
                    let connection_sessions = $connection_sessions.clone();
                    async move |input: ArtifactEditInput, cx: sacp::mcp_server::McpConnectionTo<sacp::Conductor>| {
