...
pub async fn handle_chat_message(
    ...
    *ctx.sessions.active_session_id.write().await = db_session_id.clone();
    ...
    // Native ilhae-family agents are exposed through the local A2A HTTP server, not an ACP endpoint.
    // Route relay chat directly to A2A so CLI/daemon mode doesn't stall waiting for
    // `/acp/stream` on a server that only speaks A2A HTTP.
    if is_ilhae_native_agent_id(&current_agent_id) {
        info!("[Standalone] Native ilhae-family engine selected — using direct A2A HTTP path");
