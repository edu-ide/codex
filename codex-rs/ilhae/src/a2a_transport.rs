                    ...
                    Some("session/cancel") => {
                                ...
                                || method_name.contains("message") =>
                        {
                            ...
                            info!(
                                ...
                                prompt_len = text.len(),
                                "A2A bridge forwarding prompt to A2A agent"
                            );
                            ...
                            let handle = tokio::spawn(async move {
                                ...
                                let role_section_count = response_structured
                                    .get("role_sections")
                                    .and_then(|v| v.as_array())
                                    .map(|v| v.len())
                                    .unwrap_or(0);
                                let event_count = response_structured
                                    .get("events")
                                    .and_then(|v| v.as_array())
                                    .map(|v| v.len())
                                    .unwrap_or(0);
                                info!(
                                    ...
                                    role_section_count,
                                    event_count,
                                    "A2A bridge collected terminal response"
                                );
...
///
/// Only extracts `Part::text`. `Part::data` is excluded because gemini-cli uses
/// data parts to carry thinking/thought content (description/subject fields).
...
fn collect_artifact_text(task: &Task) -> String {
    task.artifacts
        .iter()
        .map(|a| normalize_text(&extract_text_from_parts(&a.parts)))
        .filter(|t| !t.is_empty())
        .collect::<Vec<_>>()
        .join("\n")
}
    ...
    while let Some(event) = rx.recv().await {
        match event {
            Ok(StreamEvent::StatusUpdate(su)) => {
                raw_events.push(serde_json::to_value(&su).unwrap_or(Value::Null));
                ...
                if is_thought_event(meta_map) {
                    // Still collect events for observability, but skip text extraction
            ...
            Ok(StreamEvent::Task(task)) => {
                debug!(
                    ...
                    artifacts_len = task.artifacts.len(),
                    has_status_message = task.status.message.is_some(),
                    status_state = ?task.status.state,
                    "SSE StreamEvent::Task received"
                );
                if let Some(msg) = &task.status.message {
                    debug!(
                        parts_count = msg.parts.len(),
                        parts_detail = ?msg.parts.iter().map(|p| {
                            if p.text.is_some() { "text" }
                            else if p.data.is_some() { "data" }
                            else { "other" }
                        }).collect::<Vec<_>>(),
                        "SSE Task status.message parts"
                    );
                }
                ...
                raw_events.push(terminal.clone().unwrap_or(Value::Null));
                role_sections.extend(extract_role_sections_from_task(&task));
            ...
            Ok(StreamEvent::ArtifactUpdate(artifact)) => {
                raw_events.push(serde_json::to_value(&artifact).unwrap_or(Value::Null));
...
async fn collect_a2a_response(
    ...
    info!("A2A response collection started");
    match client.send_message_stream(request.clone()).await {
        Ok(mut stream_rx) => {
            ...
            info!(
                ...
                role_section_count = stream.role_sections.len(),
                event_count = stream.events.len(),
                has_terminal = stream.terminal.is_some(),
                "A2A stream collection finished"
            );
