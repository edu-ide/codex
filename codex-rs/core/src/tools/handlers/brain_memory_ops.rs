...
use crate::tools::handlers::brain_memory_ops_spec::{BRAIN_MEMORY_OPS_TOOL_NAME, create_brain_memory_ops_tool};
use crate::tools::handlers::brain_service::shared_brain_service;
use crate::tools::handlers::parse_arguments;
use crate::tools::registry::{CoreToolRuntime, ToolExecutor};
use brain_rs::{MemoryOpsRequest, execute_memory_ops};
...
impl BrainMemoryOpsHandler {
    async fn handle_call(
        ...
        let req: MemoryOpsRequest = parse_arguments(&arguments)?;
        let service = shared_brain_service()?;
        let output = tokio::task::spawn_blocking(move || execute_memory_ops(service.as_ref(), req))
            .await
...
impl CoreToolRuntime for BrainMemoryOpsHandler {}