...
#[tokio::test]
async fn goal_tools_follow_goals_feature_gate() {
    let disabled = probe(|turn| {
        set_feature(turn, Feature::Goals, /*enabled*/ false);
    })
    .await;
    disabled.assert_visible_lacks(&["get_goal", "create_goal", "update_goal"]);
    disabled.assert_registered_lacks(&["get_goal", "create_goal", "update_goal"]);
    ...
    enabled.assert_registered_contains(&["get_goal", "create_goal", "update_goal"]);
    assert_eq!(enabled.exposure("get_goal"), ToolExposure::DirectModelOnly);
    assert_eq!(
        enabled.exposure("create_goal"),
        ToolExposure::DirectModelOnly
    );
    assert_eq!(
        enabled.exposure("update_goal"),
        ToolExposure::DirectModelOnly
    );
}
...
#[tokio::test]
async fn local_web_search_is_exposed_for_provider_without_hosted_tools() {
    let probe = probe(|turn| {
        ...
        update_config(turn, |config| {
            config.model_provider_id = "oss-local".to_string();
            config.model_provider = provider_info.clone();
        });
        turn.provider = create_model_provider(provider_info, turn.auth_manager.clone());
    })
    ...
    probe.assert_visible_contains(&["web_search"]);
    let ToolSpec::Function(tool) = probe.visible_spec("web_search") else {
