...
pub(crate) fn build_tool_router(
    ...
    let hosted_specs = if crate::guardian::is_guardian_reviewer_source(&turn_context.session_source)
    {
        ...
        if should_register_local_web_search_tool(turn_context, standalone_web_search_tool.is_some())
        {
            registry.add(LocalWebSearchHandler);
        }
...
fn should_register_local_web_search_tool(
    turn_context: &TurnContext,
    standalone_web_search_available: bool,
) -> bool {
    if standalone_web_search_available || turn_context.provider.capabilities().web_search {
        return false;
    }
    !matches!(
...
fn add_core_utility_tools(context: &CoreToolPlanContext<'_>, registry: &mut ToolRegistry) {
    ...
    if features.enabled(Feature::Goals) {
        registry.add_with_exposure(GetGoalHandler, ToolExposure::DirectModelOnly);
        registry.add_with_exposure(CreateGoalHandler, ToolExposure::DirectModelOnly);
        registry.add_with_exposure(UpdateGoalHandler, ToolExposure::DirectModelOnly);
    }
    ...
    if should_register_lsp_tool(turn_context) {
        registry.register_trusted(Arc::new(LspToolHandler::new(shared_lsp_server_manager())));
    }

    registry.add(BrainVaultPatchHandler);

    if should_register_ilhae_brain_native_tools() {
        registry.add(BrainMemoryOpsHandler);
        registry.add(BrainArtifactOpsHandler);
    }
