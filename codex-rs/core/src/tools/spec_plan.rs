...
fn add_core_utility_tools(context: &CoreToolPlanContext<'_>, planned_tools: &mut PlannedTools) {
    ...
    if features.enabled(Feature::Goals) {
        planned_tools.add_with_exposure(GetGoalHandler, ToolExposure::DirectModelOnly);
        planned_tools.add_with_exposure(CreateGoalHandler, ToolExposure::DirectModelOnly);
        planned_tools.add_with_exposure(UpdateGoalHandler, ToolExposure::DirectModelOnly);
    }
