...
- `model/rerouted` — `{ threadId, turnId, fromModel, toModel, reason, backendAddress, routingReason }` when a request uses a different model. `reason` is `highRiskCyberActivity` for a safety fallback or `layaBackend` when `laya-router` reports the model selected for a response. For Laya, `backendAddress` is the selected host and port and `routingReason` is the router's decision explanation; both are `null` if the router did not provide them. Laya may emit this notification more than once per turn if it switches backends.
