...
mod ilhae_runtime;
use ilhae_runtime::RuntimeConfigTransaction;
...
impl ConfigManager {
    ...
    async fn apply_edits(
        ...
        let mut runtime_transaction = RuntimeConfigTransaction::begin(self.codex_home()).await?;
        ...
        runtime_transaction
            .capture(&provided_path, &original_user_config)
            .await?;
        ...
        for (key_path, value, strategy) in edits.into_iter() {
            ...
            ilhae_runtime::reject_direct_runtime_projection_path(&segments)?;
            if !value.is_null() {
        ...
        ilhae_runtime::ensure_runtime_projection_unchanged(&original_user_config, &user_config)?;
        ...
        if !config_edits.is_empty() {
            runtime_transaction
                .before_persist(&provided_path, &original_user_config, &user_config)
                .await?;
